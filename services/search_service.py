...
def fetch_all_patents(query_terms: list[str]) -> list[PatentRecord]:
    ...
    """
        # USPTO ODP uses a different query and response contract than
    # the legacy PatentsView service.
    if settings.patent_search_backend == "uspto":
        from services.uspto_search_service import search_patents

        log.info(
            "Using USPTO ODP backend",
            extra={"num_terms": len(query_terms)},
        )
        ...
        for term in query_terms:
            ...
            log.info(
                ...
                extra={
                    "term": term,
                    "fetched": len(results),
                    "running_total": len(all_patents),
                },
            )

        log.info(
            "USPTO fetch_all_patents complete",
            extra={"total": len(all_patents)},
        )
...
def fetch_patents_with_fallback(
    ...
    For the USPTO backend, all stages use the ODP search service directly.
    The legacy PatentsView query dictionaries are retained only for the
    legacy PatentsView backend.

    Stage 1 — validated Mistral-expanded queries
    ...
    log.info(
        "Search fallback stage 1: %d expanded queries",
    ...
    # Stage 2: individual critical tokens
    log.info(
        "Search fallback stage 2: %d critical tokens",
    ...
    log.info(
        "Search fallback complete: %d unique patents",
        len(results),
    )
